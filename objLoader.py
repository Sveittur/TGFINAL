    ...
    for line in fin.readlines():
        ...
        if tokens[0] == "mtllib":
            ...
            for i in range(vertex_count - 2):
                ...
                if len(current_uv_list) == 0:
                    mesh_model.add_vertex(current_object_id, current_position_list[int(tokens[1][0])-1], current_normal_list[int(tokens[1][2])-1],1)
                    mesh_model.add_vertex(current_object_id, current_position_list[int(tokens[i+2][0])-1], current_normal_list[int(tokens[i+2][2])-1],1)
                    mesh_model.add_vertex(current_object_id, current_position_list[int(tokens[i+3][0])-1], current_normal_list[int(tokens[i+3][2])-1],1)
                else:
